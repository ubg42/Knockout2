const unityApp = {
    ...
    loadSplitFile: async function (basePath) {
        const chunks = [];
        let totalLength = 0;
        let part = 1;
        ...
        while (true) {
            const partName =
                `${basePath}.part${String(part).padStart(2, "0")}`;
            ...
            if (!response.ok) {
                throw new Error(
                    `Failed to load ${partName}: ${response.status}`
                );
            }
        ...
        if (chunks.length === 0) {
            throw new Error(`No split parts found for ${basePath}`);
        }
        ...
        console.log(
            `Loaded ${chunks.length} parts for ${basePath}`
        );
...
const config = {
    ...
    // Set this below after joining the WASM parts
    codeUrl: "",
...
// ONLY WASM is split
const wasmBuffer = await this.loadSplitFile(
    buildUrl + "/knockout[8]-mirraSDK[5.1.2].wasm.br"
);

config.codeUrl = URL.createObjectURL(
    new Blob([wasmBuffer], {
        ...
        // Load split files
        const dataBuffer = await this.loadSplitFile(
            buildUrl + "/knockout[8]-mirraSDK[5.1.2].data.br"
        );

        // Convert the combined files into Blob URLs
        config.dataUrl = URL.createObjectURL(
            new Blob([dataBuffer], {
                type: "application/octet-stream"
            })
        );
    ...
    startLoading: async function () {
        ...
        const config = {
            arguments: [],
           dataUrl: "knockout[8]-mirraSDK[5.1.2].data.br",
           frameworkUrl: "knockout[8]-mirraSDK[5.1.2].framework.js.br",
           codeUrl: "",
            streamingAssetsUrl: "StreamingAssets",
            companyName: "DefaultCompany",
            productName: "knockout",
...
const wasmBuffer = await loadSplitFile(buildUrl + "/knockout[8]-mirraSDK[5.1.2].wasm.br");
